...
function normalizeSearchItem(item) {
  ...
  if (type === "song" || type === "video") {
    ...
    return {
      ...
      artists,
      artist: artists.length ? artists.join(", ") : item.author || null,
...
function normalizeSearchResult(result, type) {
  ...
  for (let i = 0; i < content.length && items.length < limit; i++) {
    const normalized = normalizeSearchItem(content[i]);
    if (type === "song" && normalized.type !== "song") continue;
