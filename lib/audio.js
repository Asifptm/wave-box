...
function resolveCookiesPath() {
  ...
  if (fromEnv && String(fromEnv).trim()) {
    ...
    try {
      const text = String(fromEnv).replace(/\\n/g, "\n");
...
function buildYtDlpArgs(watchUrl) {
  const args = [
    // Prefer any audio; fall back to any stream (ffmpeg will drop video)
    "-f",
    "ba/bestaudio/b",
    "-o",
    "-",
    ...
    "5",
    "--extractor-args",
    // Prefer mobile/music clients — less "sign in to confirm you're not a bot" on cloud IPs
    "youtube:player_client=android_music,android,ios,tv_embedded,web",
    "--user-agent",
...
function friendlyYtDlpError(stderrTail) {
  ...
  if (lower.includes("requested format is not available")) {
    return "No playable audio format for this video (try another track, or redeploy after format fix).";
  }
