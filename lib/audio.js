...
function buildYtDlpArgs(watchUrl) {
  const args = [
    ...
    "--fragment-retries",
    "5",
    // Point at this container's Node binary (required for YouTube JS challenges)
    "--js-runtimes",
    `node:${process.execPath}`,
    "--remote-components",
    "ejs:github",
    "--extractor-args",
    "youtube:player_client=web,android,ios,default",
    "--user-agent",
...
function friendlyYtDlpError(stderrTail) {
  ...
  if (lower.includes("sign in to confirm") || lower.includes("not a bot")) {
    return (
      "YouTube blocked this server (bot check). Add cookies on Railway: set YOUTUBE_COOKIES_B64 " +
      "from a cookies.txt export. See DEPLOY-AUDIO.md."
    );
  }
  if (lower.includes("page needs to be reloaded")) {
    return (
      "YouTube player challenge failed (page reload). Redeploy so yt-dlp can use Node JS runtime + EJS."
    );
  }
  if (lower.includes("no video formats found") || lower.includes("requested format is not available")) {
    return (
      "YouTube returned no playable formats. Re-export cookies (YOUTUBE_COOKIES_B64) " +
      "and ensure yt-dlp is current. See DEPLOY-AUDIO.md."
    );
  }
