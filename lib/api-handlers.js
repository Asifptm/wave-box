...
const { getAudioStatus, buildAudioUrl, buildStreamUrl, TTL_MINUTES, pipeAudioStream, validateVideoId, checkFfmpegAvailable, checkYtDlpAvailable, getCookiesDiagnostics } = require("./audio");
...
async function handleHealth(req, res) {
  ...
  try {
    ...
    const cookieDiag = canConvert ? getCookiesDiagnostics() : null;
    sendSuccess(res, {
      ok: true,
      service: "wavebox-api",
      ...
      cookiesBytes: cookieDiag ? cookieDiag.bytes : 0,
      cookiesLooksValid: cookieDiag ? cookieDiag.looksValid : false,
      cookiesReason: cookieDiag ? cookieDiag.reason : null,
